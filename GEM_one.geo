...
Theta_x = DefineNumber[ 0.17, Name "Parameters/theta_x" ];
//Theta_y = DefineNumber[ 0.017, Name "Parameters/theta_y" ];
//Theta_z = DefineNumber[ 0.017, Name "Parameters/theta_z" ];
...
//+ Furo 1 (centralizado em (0, Pitch/2, 0))
Cone(2) = {0, 0, -ThickDiel/2-RadiusDiel*Sin(Theta_x), 0, 0, WaistHeight*ThickDiel+RadiusDiel*Sin(Theta_x), RadiusDiel, RadiusWaist, 2*Pi};
Cone(3) = {0, 0, ThickDiel/2+RadiusDiel*Sin(Theta_x), 0, 0, (WaistHeight-1)*ThickDiel-RadiusDiel*Sin(Theta_x), RadiusDiel, RadiusWaist, 2*Pi};

Rotate { {1, 0, 0}, {0, 0, 0}, Theta_x } { Volume{2}; }
Rotate { {1, 0, 0}, {0, 0, 0}, Theta_x } { Volume{3}; }


// Aumenta o raio em 10% no eixo X
//Scale{{1.1, 1, 1}, {0, 0, 0}} { Volume{2}; };
//Scale{{1.1, 1, 1}, {0, 0, 0}} { Volume{3}; };
...
//+ Furo 1
Cylinder(12) = {0, -RadiusDiel*Sin(Theta_x), ThickDiel/2, 0, 0, ThickConduct, RadiusConduct, 2*Pi};
...
//+ Furo 1
Cylinder(18) = {0, RadiusDiel*Sin(Theta_x), -ThickDiel/2, 0, 0, -ThickConduct, RadiusConduct, 2*Pi};
...
surf_dw() = Surface In BoundingBox{  -Pitch*Sqrt(3)/2-epsilon, -epsilon, -InductionL-ThickDiel/2-ThickConduct-epsilon, Pitch*Sqrt(3)/2+epsilon, Pitch+epsilon, -InductionL-ThickDiel/2-ThickConduct+epsilon};
Physical Surface(1) = {surf_dw()};

//+ Criando Superfícies físicas do eletrodo superior
surf_up() = Surface In BoundingBox{  -Pitch*Sqrt(3)/2-epsilon, -epsilon, DriftL+ThickDiel/2+ThickConduct-epsilon, Pitch*Sqrt(3)/2+epsilon, Pitch+epsilon, DriftL+ThickDiel/2+ThickConduct+epsilon};
Physical Surface(2) = {surf_up()};

//+ Laterias do volume total
surf_t() = Surface In BoundingBox{  -Pitch*Sqrt(3)/2-epsilon, -epsilon, -InductionL-ThickDiel/2-ThickConduct-epsilon, Pitch*Sqrt(3)/2+epsilon, epsilon, DriftL+InductionL+ThickDiel+2*ThickConduct+epsilon};
Physical Surface(3) = {surf_t()};

surf_f() = Surface In BoundingBox{  -Pitch*Sqrt(3)/2-epsilon, Pitch-epsilon, -InductionL-ThickDiel/2-ThickConduct-epsilon, Pitch*Sqrt(3)/2+epsilon, Pitch+epsilon, DriftL+InductionL+ThickDiel+2*ThickConduct+epsilon};
Physical Surface(4) = {surf_f()};

surf_e() = Surface In BoundingBox{  -Pitch*Sqrt(3)/2-epsilon, -epsilon, -InductionL-ThickDiel/2-ThickConduct-epsilon, -Pitch*Sqrt(3)/2+epsilon, Pitch+epsilon, DriftL+InductionL+ThickDiel+2*ThickConduct+epsilon};
Physical Surface(5) = {surf_e()};

surf_d() = Surface In BoundingBox{  Pitch*Sqrt(3)/2-epsilon, -epsilon, -InductionL-ThickDiel/2-ThickConduct-epsilon, Pitch*Sqrt(3)/2+epsilon, Pitch+epsilon, DriftL+InductionL+ThickDiel+2*ThickConduct+epsilon};
